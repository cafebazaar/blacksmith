...
ssh-keys:
  - $(cat ~/.ssh/id_rsa.pub)
    ...
    docker run -it --name blacksmith --restart=always --net=host \
      ...
      localhost:5000/blacksmith:refactoring --verbose --config /config/$configFile
}
...
function shutdownMachine {
  local machine=$1
  vboxmanage hostonlyif remove $machine 2>/dev/null || true
  sleep 1
  # until $(VBoxManage showvminfo --machinereadable $machine | grep -q ^VMState=.poweroff.); do
  #   sleep 0.2
  # done
}
