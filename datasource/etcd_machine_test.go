...
func TestAssign(t *testing.T) {
	...
	machine1, err := ds.GetEtcdMachine(mac1).Machine(true, nil)
	if err != nil {
		t.Error("error in creating first machine:", err)
		return
	}

	machine2, err := ds.GetEtcdMachine(mac2).Machine(true, nil)
	if err != nil {
	...
	machine3, err := ds.GetEtcdMachine(mac2).Machine(true, nil)
	if err != nil {
	...
	machine4, err := ds.GetEtcdMachine(mac2).Machine(true, anIP)
	if err != nil {
	...
	_, err = ds.GetEtcdMachine(mac2).Machine(false, anIP)
	if err == nil {
	...
	_, err = ds.GetEtcdMachine(mac3).Machine(true, machine1.IP)
	if err == nil {
...
func TestLeaseRange(t *testing.T) {
	...
	for i := 1; i <= testLeaseRange; i++ {
		...
		_, err := ds.GetEtcdMachine(mac).Machine(true, nil)
